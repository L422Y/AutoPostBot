...
| Feature                | Description                           |
| ---------------------- | ------------------------------------- |
| **⚙️ Architecture**     | The codebase follows a modular architecture with generator and destination plugins. It abstracts the logic for generating and sending posts, allowing for easy extensibility and customization.   |
| **🔗 Dependencies**    | The codebase relies on external libraries such as OpenAI, Puppeteer, and Twitter API for specific functionalities. These dependencies add additional functionality and integration capabilities to the system.    |
| **🧩 Modularity**      | The system adopts a modular approach by implementing generator and destination plugins, enabling flexibility in adding new sources for post generation and destinations for posting. The use of composables also promotes code reusability.    |
| **🔌 Integrations**    | The system integrates with multiple external systems like OpenAI, Puppeteer (for Twitter automation), and Twitter APIv2. These integrations enhance the system's capabilities, allowing it to interact with various platforms.    |
| **📶 Scalability**     | The codebase has the potential for scalability due to its modular architecture. By easily adding new generator and destination plugins, the system can accommodate growth and handle increased requirements effectively.    |
...
You can control various aspects of the bot via environment variables in the `.env` file:

- `TWITTER_USERNAME` and `TWITTER_PASSWORD`: Twitter login details.
- `TWITTER_APP_KEY`, `TWITTER_APP_SECRET`, `TWITTER_ACCESS_TOKEN`, `TWITTER_ACCESS_TOKEN_SECRET`: Additional Twitter API keys and tokens for full OAuth authentication.
- `POST_MIN_DELAY_HOURS` and `POST_MAX_DELAY_HOURS`: Time delay in hours between posts.
- `ENABLED_DESTINATIONS`: Platforms where the posts will be published. For example, setting this to `TwitterAPIv2` will enable posting to Twitter using API v2.
- `ENABLED_GENERATORS`: Comma-separated list of content generators to use, along with their probabilities. For example, `FileBased:0.1,Wikipedia:0.9` enables the `FileBased` generator with a 10% chance and the `Wikipedia` generator with a 90% chance.
- `RSS_FEED_URL`: URL of the RSS feed to use with the content generator.
- `WIKIPEDIA_LANG` and `WIKIPEDIA_CATEGORIES`: Wikipedia settings for fetching random articles, including the language and category preferences.
...
- `OPENAI_INITIAL_PROMPT`: Initial prompt for generating posts via OpenAI, along with specific content guidelines like avoiding questions, gender-specific language, politics, etc.
